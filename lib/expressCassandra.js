...
CassandraClient.bind = (options, cb) => {
  ...
  CassandraClient.orm.initAsync().then(() => readdirpAsync({
...
CassandraClient.getTableList = function f(callback) {
  ...
  }).then(() => systemClient.shutdown()).then(() => {
...
CassandraClient.export = function f(fixtureDirectory, callback) {
  ...
  systemClient.connect().then(() => this.getTableListAsync()).then(tables => Promise.each(tables, table => exporter.processTableExport(systemClient, fixtureDirectory, keyspace, table))).then(() => systemClient.shutdown()).then(() => {
    debug('==================================================');
...
CassandraClient.import = function f(fixtureDirectory, options, callback) {
  ...
  systemClient.connect().then(() => this.getTableListAsync()).then(tables => Promise.each(tables, table => importer.processTableImport(systemClient, fixtureDirectory, keyspace, table, options.batchSize))).then(() => systemClient.shutdown()).then(() => {
    debug('==================================================');
...
CassandraClient._translateFileNameToModelName = fileName => fileName.slice(0, fileName.lastIndexOf('.')).replace('Model', '');
