...
UdtBuilder.prototype = {
  create_udt(typeName, typeDef, callback) {
    ...
    const query = util.format('CREATE TYPE IF NOT EXISTS "%s" (%s);', typeName, udtFields.toString());
  ...
  get_udt(typeName, keyspaceName, callback) {
    const query = util.format("SELECT * FROM system_schema.types WHERE keyspace_name = '%s' AND type_name = '%s';", keyspaceName, typeName);
    debug('executing query: %s', query);
