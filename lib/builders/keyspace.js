...
KeyspaceBuilder.prototype = {
  ...
  create_keyspace(keyspaceName, defaultReplicationStrategy, callback) {
    ...
    const query = util.format('CREATE KEYSPACE IF NOT EXISTS "%s" WITH REPLICATION = %s;', keyspaceName, replicationText);
  ...
  alter_keyspace(keyspaceName, defaultReplicationStrategy, callback) {
    ...
    const query = util.format('ALTER KEYSPACE "%s" WITH REPLICATION = %s;', keyspaceName, replicationText);
    debug('executing query: %s', query);
    this._client.execute(query, err => {
      this._client.shutdown(() => {
  ...
  get_keyspace(keyspaceName, callback) {
    const query = util.format("SELECT * FROM system_schema.keyspaces WHERE keyspace_name = '%s';", keyspaceName);
    debug('executing query: %s', query);
