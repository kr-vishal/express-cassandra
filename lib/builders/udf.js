...
UdfBuilder.prototype = {
  ...
  create_udf(functionName, functionDefinition, callback) {
    ...
    const query = util.format("CREATE OR REPLACE FUNCTION %s (%s) CALLED ON NULL INPUT RETURNS %s LANGUAGE %s AS '%s';", functionName, udfInputs.toString(), functionDefinition.returnType, functionDefinition.language, functionDefinition.code);
  ...
  get_udf(functionName, keyspaceName, callback) {
    const query = util.format("SELECT * FROM system_schema.functions WHERE keyspace_name = '%s' AND function_name = '%s';", keyspaceName, functionName.toLowerCase());
    debug('executing query: %s', query);
